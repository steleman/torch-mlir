...
export PATH="/usr/local/bin:/opt/homebrew/bin:${llvmbindir}:${PATH}"
...
gsed="/opt/homebrew/bin/sed"
cmakear="/opt/homebrew/bin/ar"
protobuf="/usr/local"
...
cmake_flags="${cmake_flags} -DUSE_CUSPARSELT=0"
cmake_flags="${cmake_flags} -DUSE_CUDSS=0"
cmake_flags="${cmake_flags} -DUSE_CUFILE=0"
cmake_flags="${cmake_flags} -DCAFFE2_USE_CUDNN:BOOL=OFF"
cmake_flags="${cmake_flags} -DCAFFE2_USE_CUSPARSELT:BOOL=OFF"
cmake_flags="${cmake_flags} -DCAFFE2_USE_CUFILE:BOOL=OFF"
...
if [ ${rc} -eq 0 ] ; then
  ts="`date +%Y%m%d%H%M%S`"
  if [ -d ./build ] ; then
    mv ./build ./build-torch-mlir-macos-wheel-${ts}
  fi

  if [ -d ./dist ] ; then
    mv ./dist ./dist-torch-mlir-macos-wheel-${ts}
  fi
else
